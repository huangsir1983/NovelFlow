"""R26 — Event 解耦流水线 (GPT-5.4 单模型).

基于 R25, 用 asyncio.Event 解耦依赖:
  R26 优化: 场景流完成即启动道具卡, 不等角色流
  R26 优化: 位置卡渐进发射不变, 场景结束立刻发射剩余批次
  R26 优化: 变体仅等角色+场景都完成后启动
  R25 保留: 双流式并行 | 弹性并发 MAX_CONCURRENT=10

用法:
    python run_r26_pipeline_test.py
"""
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """R26: Event 解耦流水线 — 场景完成即启动道具+位置卡, 不等角色."""
    print(f"\n{'=' * 60}")
    print(f"  [R26] Event 解耦流水线 — GPT-5.4")
    ...
    print(f"\n  [{status_mark}] R26 流水线完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        "status": status,
        "test_round": "R26",
        "r26_architecture": "event_decoupled_pipeline",
        "r26_optimizations": [
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R26 Event 解耦流水线验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"\n  R25 基线: 总耗时=323s")
    print(f"  R26 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    if passed:
        print(f"\n  >> R26 Event 解耦流水线验证 PASSED <<")
    else:
        print(f"\n  >> R26 Event 解耦流水线验证 FAILED <<")
        for c in checks:
            print(f"    - {c}")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r26_summary.json", result)
