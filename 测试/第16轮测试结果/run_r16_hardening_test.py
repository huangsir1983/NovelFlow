"""R16 — 流式管道加固验证 (GPT-5.4 单模型).

验证目标 (基于加固方案):
  1. 流式提取 + ProgressiveAssetParser 正常工作
  2. streaming_parser._clean() 正确剥离 markdown code fences
  3. estimate_max_tokens() GPT 分支 (1.2x 乘数)
  4. 连接池复用 (非每次新建 httpx.Client)
  5. 429/500/503 差异化重试 (由 ai_engine._call_with_retry 处理)
  6. 响应缓冲区上限 (500K 字符截断)
  7. 批量 DB flush (每5个角色/10个场景)
  8. Stage 2 并行: 位置卡 + 道具卡
  9. Stage 3: 角色变体

用法:
    python run_r16_hardening_test.py
"""
...
# ===================================================================
#  R16 加固验证: streaming_parser 单元测试
# ===================================================================

def test_streaming_parser_hardening():
    """验证 R16 加固点: code fence 剥离 + GPT token 预估."""
    print("\n  [R16 单元测试] streaming_parser 加固验证")
...
async def stage1_streaming(novel_text: str, out_dir: Path) -> tuple[list, list, dict]:
    ...
    MAX_BUFFER = 500_000  # R16: 响应缓冲区上限
    chunk_count = 0
    char_count = 0
    scene_count = 0
    ...
    if response_total_chars >= MAX_BUFFER:
        print(f"    [R16] 响应缓冲区已达上限 {MAX_BUFFER}, 后续chunks未存储")
    ...
    meta = {
        ...
        "r16_gpt_adaptive_tokens": adaptive_tokens,
    }
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """完整 Mode C 管线 — R16 加固版 (GPT-5.4 only)."""
    print(f"\n{'=' * 60}")
    print(f"  [GPT-5.4] Mode C Full Asset Pipeline (R16 加固版)")
    print(f"  加固: 缓冲区上限 | 批量flush | 429/500差异重试 | stall超时")
    print(f"  加固: code fence剥离 | GPT token适配 | event_bus | rate_limiter")
    print(f"{'=' * 60}")
    ...
    print(f"\n  [{status_mark}] GPT-5.4 Mode C R16 完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        **s1_meta,
        ...
        "r16_major_prop_top_n": MAJOR_PROP_TOP_N,
        "r16_source_text_coverage": st_coverage,
    }
    if error_msg:
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R16 流式管道加固验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"  原文%:   {result.get('r16_source_text_coverage', {}).get('coverage_pct', 0):.0f}%")
    ...
    if result.get("streaming"):
        ...
        print(f"  GPT adaptive tokens: {result.get('r16_gpt_adaptive_tokens', '?')}")
    print(f"\n  R12 基线参考: 角色=8, 场景=25, 位置=23, major道具=20, 变体=26")
    print(f"  R16 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    if result.get("num_variants", 0) < 8:
        ...
        passed = False
    cov = result.get("r16_source_text_coverage", {}).get("coverage_pct", 0)
    if cov < 80:
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r16_summary.json", result)
