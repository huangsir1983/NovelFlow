"""R24 — 双流式 + 流水线并行 (GPT-5.4 单模型).

基于 R23, 架构级重构:
  R24 核心: 角色/场景拆为两路并行流式调用, Stage 1 耗时减半
  R24 核心: 场景流式到达时渐进构建位置分组, 凑齐4个即发位置卡批次
  R24 核心: 位置卡与 Stage 1 形成流水线重叠, 不再串行等待
  R24 保留: 弹性并发 MAX_CONCURRENT=10, variant_sem=4

用法:
    python run_r24_pipeline_test.py
"""
...
_SCENE_ONLY_USER = """对以下小说执行语义切场分析。

要求：
- 完全无视原文章节，按时间-空间-情绪切场
- 每场：时空标签、在场角色、核心事件、情绪峰值、张力分数、预计时长
- 确保至少提取15个以上的叙事场景，仔细检查是否遗漏了时空跳转点
...
async def _stream_characters(novel_text: str, out_dir: Path) -> tuple[list, dict]:
    ...
    try:
        ...
        if isinstance(parsed, dict):
            parsed_chars = parsed.get("characters", [])
            if len(parsed_chars) > len(characters):
                print(f"      [角色流] 恢复: {len(characters)} -> {len(parsed_chars)}")
                characters = parsed_chars
                for i, c in enumerate(characters):
                    _save_json(chars_dir / f"char_{i:02d}_{_safe(c.get('name', ''))}.json", c)
        elif isinstance(parsed, list) and len(parsed) > len(characters):
            characters = parsed
            for i, c in enumerate(characters):
                _save_json(chars_dir / f"char_{i:02d}_{_safe(c.get('name', ''))}.json", c)
    except Exception:
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """R24: 双流式 + 流水线并行管线."""
    print(f"\n{'=' * 60}")
    print(f"  [R24] 双流式流水线 — GPT-5.4")
    print(f"  角色/场景并行流式 | 位置卡渐进发射 | 变体+道具延后并行")
    print(f"{'=' * 60}")
    ...
    # ─── Phase 1: 双流式并行 + 位置卡渐进发射 ───
    print(f"\n    [Phase 1] 双流式并行启动...", flush=True)
    t_phase1 = time.time()
    ...
    await asyncio.gather(_char_stream(), _scene_stream())

    t_phase1_done = time.time()
    print(f"\n    [Phase 1] 双流式完成: {t_phase1_done - t_phase1:.1f}s "
          f"(角色={len(characters)}, 场景={len(scenes)})", flush=True)

    # ─── Phase 2: 等位置卡完成 + 并行发变体和道具 ───
    print(f"\n    [Phase 2] 变体 + 道具 + 等待位置卡...", flush=True)
    t_phase2 = time.time()
    ...
    await asyncio.gather(_run_variants(), _run_props(), _wait_loc_batches())

    t_phase2_done = time.time()
    print(f"    [Phase 2] 完成: {t_phase2_done - t_phase2:.1f}s", flush=True)

    location_cards = loc_all_cards
    total_time = time.time() - t0_total
    status_mark = "OK" if status == "OK" else "FAIL"
    print(f"\n  [{status_mark}] R24 流水线完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        "status": status,
        "test_round": "R24",
        "r24_architecture": "dual_stream_pipeline",
        "r24_optimizations": [
        ...
        **scene_meta,
        ...
        "phase1_time_s": round(t_phase1_done - t_phase1, 2),
        "phase2_time_s": round(t_phase2_done - t_phase2, 2),
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R24 双流式流水线验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"  Phase1:  {result.get('phase1_time_s', 0):.1f}s (双流式)")
    print(f"  Phase2:  {result.get('phase2_time_s', 0):.1f}s (变体+道具+位置卡收尾)")
    ...
    print(f"\n  R23 基线: 总耗时=508s, Stage1=365s, Stage2+3=143s")
    print(f"  R24 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r24_summary.json", result)
