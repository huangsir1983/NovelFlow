...
用法:
    python run_r18_hardening_test.py
"""
...
# ===================================================================
#  R18 加固验证: streaming_parser 单元测试
# ===================================================================

def test_streaming_parser_hardening():
    """验证 R18 加固点: code fence 剥离 + GPT token 预估."""
    print("\n  [R18 单元测试] streaming_parser 加固验证")
...
async def stage1_streaming(novel_text: str, out_dir: Path) -> tuple[list, list, dict]:
    ...
    MAX_BUFFER = 500_000  # R18: 响应缓冲区上限
    chunk_count = 0
    char_count = 0
    scene_count = 0
    ...
    if response_total_chars >= MAX_BUFFER:
        print(f"    [R18] 响应缓冲区已达上限 {MAX_BUFFER}, 后续chunks未存储")
    ...
    meta = {
        ...
        "r18_gpt_adaptive_tokens": adaptive_tokens,
    }
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """完整 Mode C 管线 — R18 加固版 (GPT-5.4 only)."""
    print(f"\n{'=' * 60}")
    print(f"  [GPT-5.4] Mode C Full Asset Pipeline (R18 加固版)")
    ...
    print(f"\n  [{status_mark}] GPT-5.4 Mode C R18 完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        **s1_meta,
        ...
        "r18_major_prop_top_n": MAJOR_PROP_TOP_N,
        "r18_source_text_coverage": st_coverage,
    }
    if error_msg:
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R18 流式管道加固验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"  原文%:   {result.get('r18_source_text_coverage', {}).get('coverage_pct', 0):.0f}%")
    ...
    if result.get("streaming"):
        ...
        print(f"  GPT adaptive tokens: {result.get('r18_gpt_adaptive_tokens', '?')}")
    print(f"\n  R12 基线参考: 角色=8, 场景=25, 位置=23, major道具=20, 变体=26")
    print(f"  R18 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    if result.get("num_variants", 0) < 8:
        ...
        passed = False
    cov = result.get("r18_source_text_coverage", {}).get("coverage_pct", 0)
    if cov < 80:
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r18_summary.json", result)
