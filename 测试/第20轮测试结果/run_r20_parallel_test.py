"""R20 — 并行优化 + 截断修复 (GPT-5.4 单模型).

基于 R19 并行优化版，修复场景不足 bug:
  R20 修复: 响应截断时无条件触发场景补提取 (不再仅依赖 SCENE_MIN_THRESHOLD)
  R20 修复: SCENE_MIN_THRESHOLD 从 12 提升到 15 (与合格标准对齐)
  R19 保留: 位置卡分批并行 | Stage2+3并行 | variant_sem=4

用法:
    python run_r20_parallel_test.py
"""
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R20 并行优化验证 — GPT-5.4 (gpt-5.4)")
    ...
    # 打印最终汇总
    print(f"\n{'=' * 70}")
    print(f"  R20 测试总结 — GPT-5.4 并行优化验证")
    print(f"{'=' * 70}")
    ...
    print(f"  R20 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r20_summary.json", result)
