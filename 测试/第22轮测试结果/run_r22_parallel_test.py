"""R22 — 流式 parser buffer 截断修复 (GPT-5.4 单模型).

基于 R22, 修复 ProgressiveAssetParser buffer 截断腰斩场景对象的 bug:
  R22 修复: buffer 截断时保留正在解析的 JSON 对象, 正确计算偏移量
  R22 基线: 流式只捕获 8/21 场景 -> R22 预期全量捕获

用法:
    python run_r22_parallel_test.py
"""
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R22 并行优化验证 — GPT-5.4 (gpt-5.4)")
    ...
    # 打印最终汇总
    print(f"\n{'=' * 70}")
    print(f"  R22 测试总结 — GPT-5.4 并行优化验证")
    print(f"{'=' * 70}")
    ...
    print(f"  R22 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r22_summary.json", result)
