"""R28 — 场景精细切分修正 (GPT-5.4 单模型).

基于 R26 Event 解耦流水线:
  R28 新增: 场景切分更细粒度, 目标 25-40 个场景
  R28 新增: 强化切场规则 — 同地点不同事件必须拆分, 回忆/闪回独立成场
  R28 新增: 完整时间表记录
  R27 保留: asyncio.Event 解耦 | 渐进位置卡发射 | 弹性并发

用法:
    python run_r28_pipeline_test.py
"""
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    ...
    print(f"\n{'=' * 60}")
    print(f"  [R29] 变体提前启动 + loc_id 修复 — GPT-5.4")
    ...
    print(f"\n  [{status_mark}] R29 流水线完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        "status": status,
        "test_round": "R29",
        "r29_architecture": "variants_early_start",
        "r29_optimizations": [
    ...
    # R27: 输出时间表并保存到独立文件
    _save_json(out_dir.parent / "r29_timeline.json", timeline)
    print(f"\n  [R29 时间表]")
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R29 变体提前启动 + loc_id 修复 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"\n  R28 基线: 总耗时=418s, 场景=47个")
    print(f"  R29 合格标准: 角色>=6, 场景>=25, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r29_summary.json", result)
