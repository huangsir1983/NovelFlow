"""R25 — 双流式 + 流水线并行 复验 (GPT-5.4 单模型).

与 R25 相同架构，复验稳定性:
  双流式角色/场景并行 | 位置卡渐进发射 | 弹性并发 MAX_CONCURRENT=10

用法:
    python run_r25_pipeline_test.py
"""
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    ...
    print(f"\n  [{status_mark}] R25 流水线完成: {total_time:.1f}s 总耗时")
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R25 双流式流水线验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"  R25 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r25_summary.json", result)
