"""R19 — 并行优化验证 (GPT-5.4 单模型).

基于 R18 加固版，新增三项并行优化:
  1. 位置卡分批并行 — 24个位置分成多批(每批4-6个)并行调API
  2. Stage 2 & Stage 3 并行 — 变体生成不依赖位置卡/道具卡，与Stage2同时执行
  3. 变体并发提升 — variant_sem 从 2 提到 4

用法:
    python run_r19_parallel_test.py
"""
...
# ===================================================================
#  R19 加固验证: streaming_parser 单元测试
# ===================================================================

def test_streaming_parser_hardening():
    """验证 R19 加固点: code fence 剥离 + GPT token 预估."""
    print("\n  [R19 单元测试] streaming_parser 加固验证")
...
async def stage1_streaming(novel_text: str, out_dir: Path) -> tuple[list, list, dict]:
    ...
    MAX_BUFFER = 500_000  # R19: 响应缓冲区上限
    chunk_count = 0
    char_count = 0
    scene_count = 0
    ...
    if response_total_chars >= MAX_BUFFER:
        print(f"    [R19] 响应缓冲区已达上限 {MAX_BUFFER}, 后续chunks未存储")
    ...
    meta = {
        ...
        "r19_gpt_adaptive_tokens": adaptive_tokens,
    }
...
async def stage1_non_streaming(novel_text: str, out_dir: Path) -> tuple[list, list, dict]:
    ...
    SCENE_MIN_THRESHOLD = 12
    if len(scenes) < SCENE_MIN_THRESHOLD and len(characters) > 0:
        print(f"    [!!] 场景过少({len(scenes)}个) -> 触发独立场景提取")
        try:
            fallback_scenes = await stage1_scene_fallback(novel_text, characters, scenes_dir)
            if len(fallback_scenes) > len(scenes):
                scenes = fallback_scenes
...
async def _gen_location_cards(groups: dict, novel_text: str, out_dir: Path) -> list:
    ...
    print(f"      -> R19 位置卡分批并行: {len(groups)}个位置, 分{len(batches)}批(每批≤{BATCH_SIZE}个)...", flush=True)
    t0 = time.time()
...
async def stage3_variants(characters: list, scenes: list, out_dir: Path) -> list:
    ...
    all_variants = []
    variant_sem = asyncio.Semaphore(4)  # R19: 从2提升到4
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """完整 Mode C 管线 — R19 加固版 (GPT-5.4 only)."""
    print(f"\n{'=' * 60}")
    print(f"  [GPT-5.4] Mode C Full Asset Pipeline (R19 并行优化版)")
    print(f"  R19新增: 位置卡分批并行 | Stage2+3并行 | 变体并发x4")
    print(f"  基线加固: 缓冲区上限 | 429/500差异重试 | stall超时 | code fence剥离")
    print(f"{'=' * 60}")
    ...
    if characters or scenes:
        ...
        print(f"\n    [R19] Stage 2 + Stage 3 并行启动...", flush=True)
        t_parallel = time.time()
        await asyncio.gather(_run_stage2(), _run_stage3())
        print(f"    [R19] Stage 2+3 并行总耗时: {time.time() - t_parallel:.1f}s", flush=True)

    total_time = time.time() - t0_total
    status_mark = "OK" if status == "OK" else "FAIL"
    print(f"\n  [{status_mark}] GPT-5.4 Mode C R19 完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        "r19_optimizations": [
            "location_cards_batch_parallel_5",
        ...
        **s1_meta,
        ...
        "r19_major_prop_top_n": MAJOR_PROP_TOP_N,
        "r19_source_text_coverage": st_coverage,
    }
    if error_msg:
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R19 并行优化验证 — GPT-5.4 (gpt-5.4)")
    ...
    # 打印最终汇总
    print(f"\n{'=' * 70}")
    print(f"  R19 测试总结 — GPT-5.4 并行优化验证")
    print(f"{'=' * 70}")
    ...
    print(f"  原文%:   {result.get('r19_source_text_coverage', {}).get('coverage_pct', 0):.0f}%")
    ...
    if result.get("streaming"):
        ...
        print(f"  GPT adaptive tokens: {result.get('r19_gpt_adaptive_tokens', '?')}")
    print(f"\n  R12 基线参考: 角色=8, 场景=25, 位置=23, major道具=20, 变体=26")
    print(f"  R19 合格标准: 角色>=6, 场景>=15, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    if result.get("num_variants", 0) < 8:
        ...
        passed = False
    cov = result.get("r19_source_text_coverage", {}).get("coverage_pct", 0)
    if cov < 80:
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r19_summary.json", result)
