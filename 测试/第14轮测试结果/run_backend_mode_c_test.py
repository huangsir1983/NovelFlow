...
R14 变更 (基于 R13):
  修复一: Grok 场景文件去重 — 提取 _clean_scene_files(), 在所有写场景路径前调用
  修复二: Claude 管道重构 — 分2段非流式提取 + merge_characters/merge_scenes 合并去重
  修复三: Gemini Stage 2 冷却+重试+降级 — 15s冷却, 代理断连重试, location snippets 15条上限
...
async def run_mode_c(model_key: str, novel_text: str, out_dir: Path) -> dict:
    ...
    cfg = MODELS[model_key]
    print(f"\n{'=' * 60}")
    print(f"  [{cfg['display_name']}] Mode C Full Asset Pipeline (R14)")
    print(f"{'=' * 60}")
...
async def main():
    ...
    print(f"R14 配置: 统一流式管线, Stage2并行, 道具Top{MAJOR_PROP_TOP_N}, 场景文件去重")
    ...
    # 打印汇总
    print(f"\n{'=' * 80}")
    print(f"  Summary -- Mode C Backend Pipeline R14 (4-Model Test)")
    print(f"  R14修复: 统一流式管线 | Stage2并行 | 道具Top{MAJOR_PROP_TOP_N} | 场景去重")
    ...
    print(f"  R14 合格标准: 角色≥6, 场景≥15, 位置>0, 变体≥8, 原文覆盖≥80%, 文件数==场景数")
