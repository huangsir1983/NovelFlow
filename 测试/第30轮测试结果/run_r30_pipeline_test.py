"""R30 — 主程序后台全流水线测试 (GPT-5.4 单模型).

基于 R29 变体提前启动 + loc_id 修复流水线:
  R30: 延续 R29 全部优化, 主程序后台集成验证
  R29 保留: 变体提前启动(仅等角色) | loc_id 修复 | Event 解耦
  R28 保留: 精细切场 35-50 | 闪回独立 | 情绪转折拆分
  R27 保留: asyncio.Event 解耦 | 渐进位置卡发射 | 弹性并发

用法:
    python run_r30_pipeline_test.py
"""
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    ...
    print(f"\n{'=' * 60}")
    print(f"  [R30] 变体提前启动 + loc_id 修复 — GPT-5.4")
    ...
    result = {
        ...
        "status": status,
        "test_round": "R30",
        "r30_architecture": "variants_early_start",
        "r30_optimizations": [
    ...
    # R27: 输出时间表并保存到独立文件
    _save_json(out_dir.parent / "r30_timeline.json", timeline)
    print(f"\n  [R30 时间表]")
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R30 主程序后台测试 + loc_id 修复 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"  R30 合格标准: 角色>=6, 场景>=25, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    if passed:
        print(f"\n  >> R30 主程序后台测试验证 PASSED <<")
    else:
        print(f"\n  >> R30 主程序后台测试验证 FAILED <<")
        for c in checks:
            print(f"    - {c}")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r30_summary.json", result)
