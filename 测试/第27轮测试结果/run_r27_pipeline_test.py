"""R27 — 场景精细切分 (GPT-5.4 单模型).

基于 R26 Event 解耦流水线:
  R27 新增: 场景切分更细粒度, 目标 25-40 个场景
  R27 新增: 强化切场规则 — 同地点不同事件必须拆分, 回忆/闪回独立成场
  R27 新增: 完整时间表记录
  R26 保留: asyncio.Event 解耦 | 渐进位置卡发射 | 弹性并发

用法:
    python run_r27_pipeline_test.py
"""
...
_SCENE_ONLY_SYSTEM = (
    ...
    "5. 回忆/闪回 = 独立场景（不要和当前时空混在一起，必须单独成场）\n"
    "6. 同一地点发生的多个独立事件 = 每个事件一个场景\n"
    "7. 对话中穿插的心理活动/内心独白转折 = 可拆分\n"
    "8. 宁可多拆不要少拆，每个场景只包含一个核心动作/决定/冲突\n"
    "\n"
    "场景数量指导：目标 25-40 个叙事场景。若不足25个说明切分太粗。\n"
    "场景必须按故事发生的时间顺序排列。\n"
...
- 目标：25-40 个场景。如果你只提取了不到25个，请重新检查是否有合并过粗的场景
...
async def _stream_characters(novel_text: str, out_dir: Path) -> tuple[list, dict]:
    ...
    try:
        ...
        # 质量比较: 有 name 字段的才算有效角色
        valid_stream = sum(1 for c in characters if c.get("name"))
        valid_parsed = sum(1 for c in parsed_chars if c.get("name"))
        if valid_parsed >= valid_stream and parsed_chars:
            if valid_parsed > valid_stream:
                print(f"      [角色流] 恢复(质量): {valid_stream} -> {valid_parsed} 有效角色")
            characters = parsed_chars
            # 清理旧文件(含 unknown_* 垃圾) 再重写
...
async def _stream_scenes_progressive(
    ...
    max_tokens = 32000  # R27: 精细切分, 场景更多, 需要更多输出空间
...
async def run_mode_c(novel_text: str, out_dir: Path) -> dict:
    """R27: 场景精细切分 + Event 解耦流水线."""
    print(f"\n{'=' * 60}")
    print(f"  [R27] 场景精细切分 + Event 解耦流水线 — GPT-5.4")
    print(f"  场景完成→立刻启动道具+位置卡 | 角色完成→启动变体")
    print(f"{'=' * 60}")
    ...
    print(f"\n  [{status_mark}] R27 流水线完成: {total_time:.1f}s 总耗时")
    ...
    result = {
        ...
        "status": status,
        "test_round": "R27",
        "r27_architecture": "fine_grained_scene_split",
        "r27_optimizations": [
            "scene_fine_split_25_40",
            "flashback_independent",
            "emotion_turn_split",
            "scene_max_tokens_32k",
            "char_scene_parallel_stream",
            "progressive_loc_card_firing",
            "event_decoupled_props",
            "event_decoupled_variants",
    ...
    # R27: 输出时间表并保存到独立文件
    _save_json(out_dir.parent / "r27_timeline.json", timeline)
    print(f"\n  [R27 时间表]")
...
async def main():
    print(f"{'=' * 70}")
    print(f"  R27 场景精细切分验证 — GPT-5.4 (gpt-5.4)")
    ...
    print(f"\n  R26 基线: 总耗时=301s, 场景=16-18个")
    print(f"  R27 合格标准: 角色>=6, 场景>=25, 位置>0, 变体>=8, 原文覆盖>=80%")
    ...
    # 保存汇总
    _save_json(Path(__file__).parent / "r27_summary.json", result)
