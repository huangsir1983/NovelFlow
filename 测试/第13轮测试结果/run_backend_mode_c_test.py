...
R13 正式版变更:
  - 道具精简: major 阈值 ≥2 → ≥3, 跳过 Stage 2D minor prop visual generation
  - 道具精简: Stage 1 prompt 加入道具精简指令 (只列核心道具)
  - 场景原文定位: Stage 1 prompt 要求 source_text_start / source_text_end
  - manifest 增加 source_text 覆盖率统计
  - 跑空模型自动非流式重试
...
MODELS = {
    ...
    "grok": {
        ...
        "timeout": 600,
        "needs_stream": False,
        "max_tokens_override": 32000,  # R13: Grok max_tokens 上限
    },
}
...
# ===================================================================
#  Stage 2: 位置卡 + 道具收集 + 道具卡 (R13: 跳过 Stage 2D)
# ===================================================================

async def stage2_enrichment(model_key: str, characters: list, scenes: list,
                            novel_text: str, out_dir: Path) -> tuple[list, dict]:
    """Stage 2: Location cards + Prop collection + Prop cards.

    R13 变更:
    - major 阈值 ≥3 (从 ≥2 提升)
    - 完全跳过 Stage 2D (minor prop visual generation)
    - 返回 (location_cards, prop_data), 不再返回 minor_prop_visuals
    """
    print(f"\n    阶段2: 后处理...")
    ...
    # 2B: Prop collection — 使用后端函数，但 R13 重新分层 (major ≥3)
    raw_prop_data = collect_and_tier_props(scenes)
    # R13: 重新分层，major 阈值 ≥3
    prop_data = _retier_props(raw_prop_data, MAJOR_PROP_THRESHOLD)
    print(f"      道具 (R13 阈值≥{MAJOR_PROP_THRESHOLD}): {prop_data['total_unique']} 种 "
    ...
    # R13: 跳过 Stage 2D (minor prop visual generation)
...
async def run_mode_c(model_key: str, novel_text: str, out_dir: Path) -> dict:
    ...
    R13: manifest.json 始终写入; Stage 1 跑空自动非流式重试; 无 minor prop visuals.
    """
    cfg = MODELS[model_key]
    print(f"\n{'=' * 60}")
    print(f"  [{cfg['display_name']}] Mode C Full Asset Pipeline (R13)")
    print(f"{'=' * 60}")
    ...
    print(f"    道具 (major阈值≥{MAJOR_PROP_THRESHOLD}): {prop_data.get('total_unique', 0)} 种 "
...
def _retier_props(raw_prop_data: dict, threshold: int) -> dict:
    ...
    collect_and_tier_props 使用硬编码 ≥2，这里用 ≥threshold 重新分层。
    """
    # 合并所有道具到一个字典
    all_props = {}
...
async def main():
    ...
    print(f"R13 配置: major道具阈值≥{MAJOR_PROP_THRESHOLD}, 跳过minor道具视觉, 原文定位标记")
    ...
    # 打印汇总
    print(f"\n{'=' * 80}")
    print(f"  Summary -- Mode C Backend Pipeline R13 (4-Model Test)")
    print(f"  道具阈值: major≥{MAJOR_PROP_THRESHOLD} | minor视觉: 跳过 | 原文定位: 启用")
    print(f"{'=' * 80}")
    ...
    print(f"  R13 目标: 道具total<80, major<15, source_text覆盖>80%")
