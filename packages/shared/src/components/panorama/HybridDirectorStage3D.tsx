...
import {
  gnomonicExtractVertexShader,
  gnomonicExtractFragmentShader,
} from './gnomonicExtractShader';
import {
  rectifiedGroundVertexShader,
  rectifiedGroundFragmentShader,
} from './rectifiedGroundShader';
import {
  type Character3D,
...
/**
 * Compute gnomonic extent so the ground texture covers from nadir
 * all the way to the wall-floor junction (+ margin).
 *
 * At the wall-floor junction, the angle from nadir = atan(R / eyeLevel).
 * We add 5° margin so the texture slightly overlaps the cylinder.
 */
function computeGroundExtent(radius: number, eyeLevel: number): number {
  const angleFromNadir = Math.atan(radius / eyeLevel); // radians
  const marginRad = 5 * Math.PI / 180;
  // extent = tan(angle), capped to avoid extreme values
  return Math.min(Math.tan(angleFromNadir + marginRad), 20);
}
...
export function HybridDirectorStage3D({
  panoramaUrl,
  isOpen,
  onClose,
  ...
  const groundRTRef = useRef<THREE.WebGLRenderTarget | null>(null);
  ...
  // ══════════════════════════════════════════════════════════════
  // Gnomonic extraction: equirect → rectified ground texture
  // ══════════════════════════════════════════════════════════════
  const extractGroundTexture = useCallback((
    renderer: THREE.WebGLRenderer,
    panoramaTexture: THREE.Texture,
    extent: number,
  ): THREE.WebGLRenderTarget => {
    // Dispose previous render target
    if (groundRTRef.current) {
      groundRTRef.current.dispose();
    }

    const rt = new THREE.WebGLRenderTarget(GROUND_TEXTURE_SIZE, GROUND_TEXTURE_SIZE, {
    ...
    const extractCam = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);

    const extractGeo = new THREE.PlaneGeometry(2, 2);
    const extractMat = new THREE.ShaderMaterial({
      uniforms: {
        uPanorama: { value: panoramaTexture },
        uExtent: { value: extent },
      },
      vertexShader: gnomonicExtractVertexShader,
      fragmentShader: gnomonicExtractFragmentShader,
    });
    extractScene.add(new THREE.Mesh(extractGeo, extractMat));
    ...
    // Cleanup extraction objects
    extractGeo.dispose();
    extractMat.dispose();

    groundRTRef.current = rt;
    return rt;
  }, []);
  ...
  useEffect(() => {
    ...
    const applyTexture = (tex: THREE.Texture) => {
      ...
      // Extract gnomonic ground texture with dynamic extent based on current radius
      const extent = computeGroundExtent(radiusRef.current, PRESETS[initialSceneType].eyeLevel);
      const rt = extractGroundTexture(renderer, tex, extent);
      if (groundMatRef.current) {
        groundMatRef.current.uniforms.uGroundTexture.value = rt.texture;
  ...
  useEffect(() => {
    ...
    // ── Cylinder (walls — unchanged from original) ──
    ...
    // ── Ground plane (gnomonic-rectified texture) ──
    ...
    // Re-extract ground texture when radius changes (extent depends on R/eyeLevel)
    if (textureRef.current && rendererRef.current) {
      const extent = computeGroundExtent(R, eyeLevel);
      extractGroundTexture(rendererRef.current, textureRef.current, extent);
    }

    const gndGeo = new THREE.CircleGeometry(R, 128);
    gndGeo.rotateX(-Math.PI / 2);
    const gndMat = new THREE.ShaderMaterial({
      uniforms: {
        uGroundTexture: { value: groundRTRef.current?.texture ?? null },
      },
      vertexShader: rectifiedGroundVertexShader,
      fragmentShader: rectifiedGroundFragmentShader,
  ...
  }, [radius, sceneType, preset.eyeLevel, extractGroundTexture]);
